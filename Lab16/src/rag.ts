...
export async function compareVersions(
  ...
  const filtered = await retrieveAndAnswer(
    retriever,
    client,
    question,
    model,
    buildPolicyFilter(criteria),
  );
...
export async function retrieveAndAnswer(
  ...
  const evidence = toEvidenceChunks(await retriever.retrieve(question, filter));
  if (evidence.length === 0) {
    return { evidence, answer: { text: ABSTENTION, citations: [], abstained: true } };
  }
...
export function buildGroundedRequest(
  ...
  const documents: Anthropic.Messages.ContentBlockParam[] = evidence.map((chunk) => ({
    ...
    context: [
      ...
      `Representation: ${chunk.metadata.representation}`,
...
export function readCitedAnswer(response: ModelResponse, evidence: EvidenceChunk[]): CitedAnswer {
  ...
  if (plainText.startsWith(ABSTENTION)) {
  ...
  return { text, citations: [...new Set(filenames)], abstained: false };
}
