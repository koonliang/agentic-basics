import { Anthropic } from "./instrumentation.js";
...
export async function coordinate(input: CoordinatorInput): Promise<string> {
  ...
  emit(createTrace({
    ...
    durationMs: Date.now() - routingStarted,
  }));
  ...
  const settled = await Promise.allSettled(uniqueDelegations.map(async (delegation) => {
    ...
    try {
      const toolName = delegation.id === "order-investigator"
        ? "delegate_order_investigator"
        : "delegate_policy_specialist";
      const result = await withToolSpan(
        toolName,
        delegation.task,
        () => input.transport.send(selected.agent, delegation.task, emit),
      );
      emit(createTrace({
