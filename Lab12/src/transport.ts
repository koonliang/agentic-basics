...
import { currentTraceHeaders, type TraceHeaders } from "./observability.js";
...
export function createA2ATransport(options: TransportOptions): A2ATransport {
  ...
  const fetchImpl = tracedFetch(options.fetchImpl ?? fetch);
...
export function createAgentCoreInvocationInput(
  ...
  runtimeUserId: string,
  streaming = false,
  traceHeaders: TraceHeaders = currentTraceHeaders(),
...
function tracedFetch(fetchImpl: typeof fetch): typeof fetch {
  return (input, init = {}) => {
    const headers = new Headers(init.headers);
    const tracing = currentTraceHeaders();
    if (tracing.traceParent) headers.set("traceparent", tracing.traceParent);
    if (tracing.traceState) headers.set("tracestate", tracing.traceState);
    if (tracing.baggage) headers.set("baggage", tracing.baggage);
