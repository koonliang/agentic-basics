...
import { recordTraceMetric, traceCorrelation } from "./observability.js";
...
export function logTrace(event: TraceEvent, sessionId?: string): void {
  recordTraceMetric(event);
  console.log(JSON.stringify({ event: "orchestration_trace", ...traceCorrelation(sessionId), ...event }));
}
