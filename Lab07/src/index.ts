...
import { createCoordinatorOptions, eventsFromMessage, type AgentEvent } from "./agent.js";

const defaultPrompt = [
  "Investigate cases/case-001.md.",
  "Use both specialists to verify the order facts and applicable refund policy, then recommend the next support action.",
].join(" ");
...
for await (const message of query({
  prompt,
  options: createCoordinatorOptions(workspace, model),
})) {
...
function printEvent(event: AgentEvent): void {
  if (event.type === "init") {
    ...
    console.log(`[${event.source} tool result] id=${event.toolUseId} error=${event.isError}`);
