...
import { buildMetadataSidecar, getCorpus } from "./corpus.js";
...
for (const document of corpus) {
  await s3.send(new PutObjectCommand({
    ...
    Key: document.key,
    Body: await readFile(document.sourcePath),
    ContentType: "application/pdf",
  }));
  await s3.send(new PutObjectCommand({
    Bucket: config.documentBucket,
    Key: document.metadataKey,
    Body: buildMetadataSidecar(document),
    ContentType: "application/json",
  }));
  console.log(`Uploaded s3://${config.documentBucket}/${document.key} with metadata`);
}
...
const started = await bedrock.send(new StartIngestionJobCommand({
  ...
  description: `Lab15 ingestion of ${corpus.length} versioned policy documents`,
}));
